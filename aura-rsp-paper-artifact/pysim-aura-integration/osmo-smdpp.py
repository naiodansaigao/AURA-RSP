...
from pySim.esim.profile_store import ProfileRepository, ProfileStoreError # noqa: E402
...
class SmDppHttpServer:
    ...
    def __init__(self, server_hostname: str, ci_certs_path: str, common_cert_path: str,
                 ...
                 rsp_mode: str = 'standard', aura_root: Optional[str] = None):
        if rsp_mode not in ('standard', 'aura'):
            raise ValueError(f'Unsupported RSP mode: {rsp_mode}')
        self.rsp_mode = rsp_mode
        self.server_hostname = server_hostname
        self.upp_dir = os.path.realpath(os.path.join(DATA_DIR, 'upp'))
        self.profile_repository = ProfileRepository(self.upp_dir)
        ...
        self.aura_service = None
        self.aura_store = None
        self._aura_pr_nonces = set()
        if rsp_mode == 'aura':
            if aura_root is None:
                raise ValueError('aura_root is required in AURA mode')
            from pySim.esim.aura.bootstrap import seed_store
            from pySim.esim.aura.service import AuraService
            from pySim.esim.aura.store import AuraStore

            aura_root_path = Path(aura_root).resolve()
            if in_memory:
                self.aura_store = AuraStore(in_memory=True)
            else:
                aura_db = aura_root_path / 'runtime' / 'aura' / 'sessions'
                self.aura_store = AuraStore(filename=str(aura_db), in_memory=False)
            seed_store(self.aura_store, aura_root_path)
            self.aura_service = AuraService(
                root=aura_root_path,
                profile_repository=self.profile_repository,
                store=self.aura_store,
            )
            self._aura_pr_key = (
                aura_root_path / 'runtime' / 'aura' / 'pr-shared-key.bin'
            ).read_bytes()

    def _require_standard(self):
        if self.rsp_mode != 'standard':
            raise ApiError('8.10.1', '3.8', 'Standard ES9+ disabled in AURA mode')

    def _aura_pr_identity(self, request: IRequest, path: str, content: dict) -> str:
        if self.rsp_mode != 'aura' or self.aura_service is None:
            raise RuntimeError('AURA mode is not enabled')
        praddr = request.getHeader('X-AURA-PRADDR')
        nonce = request.getHeader('X-AURA-PR-NONCE')
        supplied = request.getHeader('X-AURA-PR-AUTH')
        if not praddr or not nonce or not supplied or nonce in self._aura_pr_nonces:
            raise ValueError('AURA PR authentication missing or replayed')
        authenticated = {
            'path': path,
        ...
        if not hmac.compare_digest(expected, supplied):
            raise ValueError('AURA PR authentication failed')
        self._aura_pr_nonces.add(nonce)
        if len(self._aura_pr_nonces) > 10000:
            self._aura_pr_nonces.clear()
            self._aura_pr_nonces.add(nonce)
        return praddr
    ...
    def authenticateClient(self, request: IRequest, content: dict) -> dict:
        ...
        if euiccSigned1['ctxParams1'][0] == 'ctxParamsForCommonAuthentication':
            ...
            if matchingId:
                try:
                    profile_record = self.profile_repository.load(matchingId)
                except ProfileStoreError:
                    raise ApiError('8.2.6', '3.8', 'Refused')
                ss.matchingId = matchingId
                iccid_str = profile_record.iccid_hex
        else:
        ...
        ss.profileMetadata = profile_record.standard_metadata(self.server_hostname)
    ...
    def getBoundProfilePackage(self, request: IRequest, content: dict) -> dict:
        ...
        profile_record = self.profile_repository.load(ss.matchingId)
        upp = UnprotectedProfilePackage.from_der(
            profile_record.data, metadata=ss.profileMetadata
        )
    ...
    def _aura_dispatch(self, request: IRequest, path: str, mandatory: List[str],
                       allowed: List[str], handler, success_status: int = 200):
        from pySim.esim.aura.errors import AuraProtocolError
        from pySim.esim.aura.http_api import (
            decode_json_request,
            encode_json_response,
        )

        if self.rsp_mode != 'aura':
            return encode_json_response(
                request, 404, {'error': 'AURA_MODE_DISABLED', 'stage': 'routing'}
            )
        try:
            content = decode_json_request(
                request, mandatory=mandatory, allowed=allowed
            )
            pr_identity = self._aura_pr_identity(request, path, content)
            result = handler(content, pr_identity)
            return encode_json_response(request, success_status, result)
        except AuraProtocolError as exc:
            logger.warning(
                'AURA rejected path=%s stage=%s code=%s',
                path, exc.stage, exc.code
            )
            return encode_json_response(
                request,
                exc.http_status,
                {'error': exc.code, 'stage': exc.stage},
            )
        except ValueError as exc:
            logger.warning('AURA PR rejected path=%s error=%s', path, exc)
            return encode_json_response(
                request, 403,
                {'error': 'PR_AUTHENTICATION_FAILED', 'stage': 'transport'},
            )
        except Exception as exc:
            logger.exception('AURA internal error path=%s', path)
            return encode_json_response(
                request, 500,
                {'error': 'INTERNAL_ERROR', 'stage': 'server'},
            )
    ...
    def health(self, request: IRequest):
        ...
        return json.dumps({
            'status': 'ok',
            'rspMode': self.rsp_mode,
            'server': 'osmo-smdpp',
        })

    @app.route('/aura/rsp/v1/initiateAuthentication', methods=['POST'])
    def auraInitiateAuthentication(self, request: IRequest):
        path = '/aura/rsp/v1/initiateAuthentication'
        return self._aura_dispatch(
            request,
            path,
            ['I_ac', 'N_U', 'capabilities'],
            ['I_ac', 'N_U', 'capabilities'],
            self.aura_service.initiate if self.aura_service else None,
        )

    @app.route('/aura/rsp/v1/authenticateClient', methods=['POST'])
    def auraAuthenticateClient(self, request: IRequest):
        path = '/aura/rsp/v1/authenticateClient'
        fields = [
            'transactionId', 'salt_p', 'lph', 'nu', 'opid', 'vk_t',
            'cred_exp', 'gamma', 'c', 'tau_auth', 'Pi_auth',
        ]
        return self._aura_dispatch(
            request,
            path,
            fields,
            fields,
            self.aura_service.authenticate if self.aura_service else None,
        )

    @app.route('/aura/rsp/v1/getBoundProfilePackage', methods=['POST'])
    def auraGetBoundProfilePackage(self, request: IRequest):
        path = '/aura/rsp/v1/getBoundProfilePackage'
        fields = [
            'transactionId', 'Bind_t', 'ctx_bind', 'mode', 'Q_U', 'sigma_U_Q',
        ]
        return self._aura_dispatch(
            request,
            path,
            fields,
            fields,
            self.aura_service.get_profile if self.aura_service else None,
        )

    @app.route('/aura/rsp/v1/handleNotification', methods=['POST'])
    def auraHandleNotification(self, request: IRequest):
        path = '/aura/rsp/v1/handleNotification'
        fields = ['transactionId', 'InstallReceipt']
        return self._aura_dispatch(
            request,
            path,
            fields,
            fields,
            self.aura_service.notification if self.aura_service else None,
            success_status=204,
        )

    @app.route('/aura/rsp/v1/prepareLifecycleOperation', methods=['POST'])
    def auraPrepareLifecycleOperation(self, request: IRequest):
        path = '/aura/rsp/v1/prepareLifecycleOperation'
        fields = [
            'transactionId', 'Bind_t', 'ctx_bind', 'mode', 'Q_U', 'sigma_U_Q',
        ]
        return self._aura_dispatch(
            ...
            fields,
            fields,
            self.aura_service.prepare_lifecycle if self.aura_service else None,
        )

    @app.route('/aura/rsp/v1/handleLifecycleReceipt', methods=['POST'])
    def auraHandleLifecycleReceipt(self, request: IRequest):
        path = '/aura/rsp/v1/handleLifecycleReceipt'
        fields = ['transactionId', 'StateReceipt']
        return self._aura_dispatch(
            request,
            path,
            fields,
            fields,
            self.aura_service.lifecycle_receipt if self.aura_service else None,
        )

    @app.route('/aura/rsp/v1/commitDelete', methods=['POST'])
    def auraCommitDelete(self, request: IRequest):
        path = '/aura/rsp/v1/commitDelete'
        fields = ['transactionId', 'CommitReceipt']
        return self._aura_dispatch(
            request,
            path,
            fields,
            fields,
            self.aura_service.commit_delete if self.aura_service else None,
        )

    @app.route('/aura/rsp/v1/handleReinstallReceipt', methods=['POST'])
    def auraHandleReinstallReceipt(self, request: IRequest):
        path = '/aura/rsp/v1/handleReinstallReceipt'
        fields = ['transactionId', 'ReinstallReceipt']
        return self._aura_dispatch(
            request,
            path,
            fields,
            fields,
            self.aura_service.reinstall_receipt if self.aura_service else None,
        )

    @app.route('/aura/rsp/v1/getLifecycleState', methods=['POST'])
    def auraGetLifecycleState(self, request: IRequest):
        path = '/aura/rsp/v1/getLifecycleState'
        fields = ['lph']
        return self._aura_dispatch(
            request,
            path,
            fields,
            fields,
            self.aura_service.get_lifecycle_state if self.aura_service else None,
        )
...
def main(argv):
    ...
    parser.add_argument("-p", "--port", help="TCP port to bind HTTP(S) to", type=int, default=443)
    ...
    parser.add_argument("-m", "--in-memory", help="Use ephermal in-memory session storage (for concurrent runs)",
                        action='store_true', default=False)
    parser.add_argument("--rsp-mode", choices=("standard", "aura"),
                        default="standard",
                        help="Select Standard ES9+ or AURA-RSP research mode")
    parser.add_argument("--aura-root", default=str(Path(__file__).resolve().parent),
                        help="Integration root containing config/ and runtime/aura/")
    ...
    hs = SmDppHttpServer(
        ...
        rsp_mode=args.rsp_mode,
        aura_root=args.aura_root,
    )
    print(f"OSMO_SMDPP_MODE={args.rsp_mode}")
    if(args.nossl):
